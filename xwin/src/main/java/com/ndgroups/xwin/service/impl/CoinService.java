...
//    @Autowired
    ...
    public String getMarketChat(String coinId, int days) throws Exception {
        String url  = "https://api.coingecko.com/api/v3/coins/"+ coinId +"/market_chart?vs_currency=usd" +
                "&days"+ days;
        RestTemplate restTemplate = new RestTemplate();
        try {
            HttpHeaders headers = new HttpHeaders();
    ...
    public String getCoinDetails(String coinId) throws Exception {
        ...
        try {
            ...
            coin.setHigh24h(marketData.get("hign_24h").get("usd").asDouble());
            coin.setLow24h(marketData.get("low_24h").get("usd").asDouble());
            coin.setPriceChange24h(marketData.get("price_change_24h").get("usd").asDouble());
            coin.setPriceChangePercentage24h(marketData.get("price_change_percentage_24h")
                    .get("usd").asDouble());
            coin.setMarketCapChange24h(marketData.get("market_cap_change_24h")
                    .get("usd").asLong());
            coin.setMarketCapChangePercentage24h(marketData.get("market_cap_change_percentage_24h")
                    .get("usd").asLong());
    ...
    @Override
    public Coin getCoinById(String coinId) throws Exception {
        return coinRepository.findById(coinId)
                .orElseThrow(() -> new Exception("coin with Id not found"));
    }
    ...
    @Override
    public String getTop50CoinByMarketTopRank() throws Exception {
        String url  = "https://api.coingecko.com/api/v3/coins/market/vs_currency=usd&per_page=50&" +
                "page=1";
        RestTemplate restTemplate = new RestTemplate();
        try {
            HttpHeaders headers = new HttpHeaders();
